  ...
  <div class="clearfix selector">
    ...
    <div class="type-wrap" v-for="attrs in attrsList" :key="attrs.attrId">
      ...
      <div class="fl value">
        <!-- 平台售卖属性：值 -->
        <ul class="type-list">
          <li v-for="(attrValue,index) in attrs" :key="index" @click="attrInfo(attrs,attrValue)">
            <a>{{attrValue}}</a>
          </li>
        </ul>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      tradeMatkhandler(trademark){
        // 点击了品牌（苹果），需要整理参数，向服务器发请求获取相应的数据进行展示
    ...
    computed:{
      ...mapGetters("searchOptions",['trademarkList','attrsList'])
    }
  }
</script>
