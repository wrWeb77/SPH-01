          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd @click="changeActive(spuSaleAttrValue,spuSaleAttr.spuSaleAttrValueList)" changepirce="0" :class="{active:spuSaleAttrValue.isChecked == 1}" v-for="spuSaleAttrValue in spuSaleAttr.spuSaleAttrValueList" :key="spuSaleAttrValue.id">{{spuSaleAttrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
              ...
              <div class="add" >
                <!-- 以前都是a路由跳转到b 路由这里在加入购物车，进行路由跳转之前，发请求
                     把购买的产品的信息通过请求的形式通知服务器，服务器进行相应的储存
                 -->
...
<script>
  ...
  export default {
    ...
    computed:{
      ...mapGetters("detailOptions",["spuSaleAttrList","categoryView","skuInfo"]),
      ...mapState("detailOptions",["goodInfo"]),
      skuImageList(){
    ...
    methods: {
      ...
      // 加入购物车的回调
      async addShopcar(){
        // 派发请求 将产品加入到数据库（通知服务器）
        // 服务器存储成功=== 进行路由跳转（传递参数）
        // 失败，对用户进行提示
      // 判断 加入购物车是否添加成功  如果成功进行 路由跳转 否则给用户提示
        try {
          // 成功
          await this.$store.dispatch('detailOptions/addOrUpdateShirCart',{skuId:this.$route.params.skuid,skuNum:this.skuNum})
          // 在路由跳转的时候将产品的信息带给 add组件
          // 产品信息的数据【比较复杂：skuInfo】 通过session 会话存储传递
    ...
    mounted() {
      // 派发actions 获取产品详情的信息
      this.$store.dispatch('detailOptions/getGoodInfo',this.$route.params.skuid)
    },
  }
</script>
