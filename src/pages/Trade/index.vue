  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        ...
        <ul
          class="list clearFix"
          v-for="detailArrayList in orderInfo.detailArrayList"
          :key="detailArrayList.skuId"
        >
          <li>
            <img
              :src="detailArrayList.imgUrl"
              alt=""
              style="width: 82px; height: 82px"
            />
          </li>
          <li>
            <p>{{ detailArrayList.skuName }}</p>
            <h4>7天无理由退货</h4>
          </li>
          <li>
            <h3>￥{{ detailArrayList.orderPrice }}.00</h3>
          </li>
          <li>X{{ detailArrayList.skuNum }}</li>
          <li>有货</li>
        </ul>
      </div>
      <div class="bbs">
...
export default {
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      let data = {
        ...
        orderDetailList:this.orderInfo.detailArrayList // 订单清单
      };
  ...
  computed: {
    ...mapState("tradeOptions", ["addressInfo", "orderInfo"]),
    // 将来提交订单最终选中的地址
    userDefaultaddress() {
      // find() 查找数据当中符合条件的元素返回，组后最终结果
      return this.addressInfo.find((item) => item.isDefault == 1) || {};
    },
  },
  mounted() {
    this.$store.dispatch("tradeOptions/getUserAdddress");
    this.$store.dispatch("tradeOptions/getOrderInfo");
  },
};
</script>
